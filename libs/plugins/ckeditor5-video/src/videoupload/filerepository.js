 ...
 */
export default class VideoFileRepository extends Plugin {
    /**
     * @inheritDoc
     */
...
mix( VideoFileRepository, ObservableMixin );
...
class FileLoader {
    ...
    read() {
        if ( this.status != 'idle' ) {
            throw new CKEditorError( 'videofilerepository-read-wrong-status: You cannot call read if the status is different than idle.' );
        }
    ...
    upload() {
        if ( this.status != 'idle' ) {
            throw new CKEditorError( 'videofilerepository-upload-wrong-status: You cannot call upload if the status is different than idle.' );
        }
